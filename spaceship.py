...
def process_sprite_group(a_set, canvas):
    ...
    for sprite in copy_of_set:
        sprite.draw(canvas)
        if not sprite.update():
            a_set.discard(sprite)
...
def group_collide(group, other_object):
    ...
    for sprite in group_copy:
        if sprite.collide(other_object):
            group.remove(sprite)
            explosion = Sprite(sprite.pos, [0, 0], 0, 0,
                               explosion_image,
                               explosion_info,
                               explosion_sound)
...
class Sprite:
    ...
    def update(self):
        ...
        # update age
        self.age += 1
        keep_the_sprite = True
        if self.age >= self.lifespan:
            keep_the_sprite = False
        return keep_the_sprite
...
def rock_spawner():
    if len(rock_group) < 12:
        ...
        if (dist(a_rock.pos, my_ship.pos) >
            2 * (a_rock.get_radius() + my_ship.get_radius())
            and started):
